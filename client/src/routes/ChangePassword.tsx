    ...
    switch (error.message) {
      case "Invalid userName":
        return {
          ...error,
          type: "error",
          content: "common.invalid_userName",
        };
...
const  ChangePassword = ({ open, handleClose }: ChangePasswordProps) => {
    ...
    const handlePasswordChange = useCallback(
        (event: React.FormEvent<HTMLFormElement>) => {
          ...
          if(  data.get("old_password1") ===  data.get("old_password2")){
            const user_data = {
              username: data.get("username"),
              old_password: data.get("old_password"),
              new_password: data.get("old_password1"),
            };
            console.log('handlePasswordChange', user_data);
            onValid(user_data);
          }else{
